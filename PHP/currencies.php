<?php
    ...
    $url = 'https://valorant-api.com/v1/currencies?language=pt-BR';
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    $resultado = json_decode(curl_exec($ch));
    $moedas = $resultado->data;
    $totalMoedas = count($moedas);
    $moedasPerPage = 12;
    $totalPages = ceil($totalMoedas / $moedasPerPage);
    $currentPage = isset($_GET['page']) ? $_GET['page'] : 1;
    $start = ($currentPage - 1) * $moedasPerPage;
    $currentMoedas = array_slice($moedas, $start, $moedasPerPage);
?>
<!DOCTYPE html>
<html lang="pt-BR">
    <head>
    ...
    <body>
        <div class="container">
            ...
            <h2 class="mt-4">Moedas</h2>
            <div class="row">
                <?php
                    foreach($currentMoedas as $moeda){
                        $id = str_replace("https://valorant-api.com/v1/currencies/", "", $moeda->uuid);
                ?>
                <div class="col-md-3 mb-4">
                    <div class="card" style="width: 18rem;">
                        <img class="card-img-top" src="<?php echo $moeda->displayIcon; ?>" alt="Imagem da moeda <?php echo $moeda->displayName; ?>">
                        <div class="card-body">
                            <h5 class="card-title" style="color: #ff4655"><?php echo $moeda->displayName; ?></h5>
                        </div>
                    </div>
                </div>
                <?php
