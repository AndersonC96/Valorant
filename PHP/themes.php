<?php
    ...
    $url = 'https://valorant-api.com/v1/themes?language=pt-BR';
    $ch = curl_init($url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    $resultado = json_decode(curl_exec($ch));
    $themes = $resultado->data;
    $totalThemes = count($themes);
    $themesPerPage = 12;
    $totalPages = ceil($totalThemes / $themesPerPage);
    $currentPage = isset($_GET['page']) ? $_GET['page'] : 1;
    $start = ($currentPage - 1) * $themesPerPage;
    $currentThemes = array_slice($themes, $start, $themesPerPage);
?>
<!DOCTYPE html>
<html lang="pt-BR">
    <head>
    ...
    <body>
        <div class="container">
            ...
            <h2 class="mt-4">Temas</h2>
            <div class="row">
                <?php
                    foreach($currentThemes as $theme){
                        $id = str_replace("https://valorant-api.com/v1/themes/", "", $theme->uuid);
                ?>
                <div class="col-md-3 mb-4">
                    <div class="card" style="width: 18rem;">
                        <img class="card-img-top" src="<?php echo $theme->displayIcon; ?>" alt="Imagem do tema <?php echo $theme->displayName; ?>">
                        <div class="card-body">
                            <h5 class="card-title" style="color: #ff4655"><?php echo $theme->displayName; ?></h5>
                        </div>
                    </div>
                </div>
                <?php
